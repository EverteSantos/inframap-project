...
@router.post('/', response_model = TipoAtivoResponse)
...
def listar_tipo_ativos(db: Session = Depends(get_db)):

    lista_tipo_ativos = db.query(TipoAtivo).all()


    return lista_tipo_ativos

@router.get('/{tipoa_ativo_id}', response_model=TipoAtivoResponse)
def busca_por_id(tipoa_ativo_id: int, db: Session = Depends(get_db)):

    busca_ativo_id = db.query(TipoAtivo).filter(TipoAtivo.id == tipoa_ativo_id).first()
...
@router.put('/', response_model=TipoAtivoResponse)
def 